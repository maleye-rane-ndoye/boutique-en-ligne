...
class METOD
{
    ...
    //     //  methode pour verifier l'existance d'un login dans la BDD.
     public function verif_bdd($email){

         $data = $this->bdd->prepare("SELECT * FROM `admin` WHERE email=?");
         $data->execute(["$email"]);
    ...
    //     // methode pour enregistrer un user dans la BDD
     public function register($nom, $prenom, $email, $password){
        ...
        $req = $this->bdd->prepare("INSERT INTO `admin`(`nom`, `prenom`, `email`, `password`) VALUES (:nom, :perenom, :email, :password);");
    ...
    //     $_SESSION = array();//Ecraser le tableau de session 
    //     session_unset(); //Detruit toutes les variables de la session en cours
    //     session_destroy();//Destruit la session en coursclass Userpdo:{
    //         public $login;
    //         private $password;
    //         public $email;
    //         public $firstname;
